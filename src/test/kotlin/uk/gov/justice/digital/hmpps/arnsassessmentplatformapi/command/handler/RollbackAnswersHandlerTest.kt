...
class RollbackAnswersHandlerTest {
  ...
  val handler = RollbackAnswersHandler(
  ...
  fun `it stores the type of the command it is built to handle`() {
    assertThat(handler.type).isEqualTo(RollbackAnswers::class)
  }

  @Test
  fun `it handles the RollbackAnswers command`() {
    val command = RollbackAnswers(
    ...
    val eventData = assertIs<AnswersRolledBack>(event.captured.data)

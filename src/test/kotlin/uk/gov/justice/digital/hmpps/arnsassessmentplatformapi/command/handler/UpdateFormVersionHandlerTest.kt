...
class UpdateFormVersionHandlerTest {
  ...
  fun `it stores the type of the command it is built to handle`() {
    assertThat(handler.type).isEqualTo(UpdateFormVersion::class)
  }

  @Test
  fun `it handles the UpdateFormVersion command`() {
    val command = UpdateFormVersion(
    ...
    val eventData = assertIs<FormVersionUpdated>(event.captured.data)

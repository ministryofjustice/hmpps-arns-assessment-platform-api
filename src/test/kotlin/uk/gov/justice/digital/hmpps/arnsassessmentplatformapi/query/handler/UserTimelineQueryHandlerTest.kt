...
class UserTimelineQueryHandlerTest {
  ...
  val handler = UserTimelineQueryHandler(services)
  ...
  fun `returns the timeline for a given timeframe`() {
    ...
    every {
      services.timeline.findAllBetweenByUserUuid(
        userEntity.uuid,
        timeframe.from,
        timeframe.to,
      )
    ...
    every {
      services.userDetails.find(user)
    } returns userEntity

    val query = UserTimelineQuery(
      user = user,
      timestamp = now,
      subject = user,
      window = timeframe,
  ...
  fun `returns a timeline containing a specified number of items`() {
    ...
    every {
      services.timeline.findAllPageableByUserUuid(
        userEntity.uuid,
        count,
        pageNumber,
      )

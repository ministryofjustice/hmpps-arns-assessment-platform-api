...
class GetAggregateTest(
  ...
  fun `it fetches the latest aggregate for an assessment`() {
    ...
    val result = webTestClient.get().uri("/aggregate/${AssessmentVersionAggregate.aggregateType}/${assessment.uuid}")
  ...
  fun `it fetches an aggregate for a point in time`() {
    ...
    val result = webTestClient.get()
      .uri("/aggregate/${AssessmentVersionAggregate.aggregateType}/${assessment.uuid}?timestamp=2025-01-01T12:15:00")
      .headers(setAuthorisation(roles = listOf("ROLE_ARNS_ASSESSMENT_PLATFORM_READ")))
      .exchange()
      .expectStatus().isOk
      .expectBody(AssessmentVersionResponse::class.java)
  ...
  fun `it returns 404 when there is no assessments`() {
    webTestClient.get().uri("/aggregate/${AssessmentVersionAggregate.aggregateType}/${UUID.randomUUID()}")
      .headers(setAuthorisation(roles = listOf("ROLE_ARNS_ASSESSMENT_PLATFORM_READ")))
      .exchange()

...
import uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.command.handler.UpdateAnswersCommandHandler
...
class UpdateAnswersCommandHandlerTest {
  ...
  fun `it stores the type of the command it is built to handle`() {
    assertThat(handler.type).isEqualTo(UpdateAnswers::class)
  }

  @Test
  fun `it handles the UpdateAnswers command`() {
    val command = UpdateAnswers(
    ...
    val eventData = assertIs<AnswersUpdated>(event.captured.data)

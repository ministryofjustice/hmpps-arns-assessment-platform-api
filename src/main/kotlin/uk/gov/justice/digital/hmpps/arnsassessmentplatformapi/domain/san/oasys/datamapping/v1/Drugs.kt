...
class Drugs : SectionMapping() {
  ...
  private fun getUsageFrequencyScore(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field): Any? = when (ap.answer(field).value) {
  ...
  private fun isUsing(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field, frequencies: Set<uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Value>): Boolean {
  ...
  private fun isMoreThanSix(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field): Boolean = ap.answer(field).value
    .equals(ap.get(Value.MORE_THAN_SIX))

  private fun isLastSix(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field): Boolean = ap.answer(field).value
    .equals(ap.get(Value.LAST_SIX))

  private fun containsMoreThanSix(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field): Boolean = ap.answer(field).values.orEmpty()
    .contains(ap.get(Value.MORE_THAN_SIX))

  private fun containsLastSix(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field): Boolean = ap.answer(field).values.orEmpty()
    .contains(ap.get(Value.LAST_SIX))

  private fun hasInjected(value: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Value): Boolean = ap.answer(
    Field.DRUGS_INJECTED,
  ).values.orEmpty().contains(ap.get(value))

  private fun yesIfContains(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field, value: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Value): String? = ap.answer(field).values?.let {

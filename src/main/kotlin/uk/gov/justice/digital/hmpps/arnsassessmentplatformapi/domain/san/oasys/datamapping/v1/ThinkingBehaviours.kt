...
class ThinkingBehaviours(ap: AnswersProvider) : SectionMapping(ap) {
  ...
  private fun q2(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_IMPULSIVE_BEHAVIOUR).value) {
    ap.get(Value.NO) -> "0"
  ...
  private fun q3(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_VIOLENCE_CONTROLLING_BEHAVIOUR).value) {
  ...
  private fun q4(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_TEMPER_MANAGEMENT).value) {
    ap.get(Value.YES) -> "0"
    ap.get(Value.SOMETIMES) -> "1"
    ap.get(Value.NO) -> "2"
    else -> null
  ...
  private fun q6(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_PROBLEM_SOLVING).value) {
    ap.get(Value.YES) -> "0"
  ...
  private fun q7(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_CONSEQUENCES).value) {
    ap.get(Value.YES) -> "0"
    ap.get(Value.SOMETIMES) -> "1"
    ap.get(Value.NO) -> "2"
    else -> null
  ...
  private fun q9(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_PEOPLES_VIEWS).value) {
    ap.get(Value.YES) -> "0"
    ap.get(Value.SOMETIMES) -> "1"
    ap.get(Value.NO) -> "2"
    else -> null
  ...
  private fun q11(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_SEXUAL_PREOCCUPATION).value) {
    ap.get(Value.YES) -> "2"
    ap.get(Value.SOMETIMES) -> "1"
    ap.get(Value.NO) -> "0"
    else -> null
  ...
  private fun q12(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_OFFENCE_RELATED_SEXUAL_INTEREST).value) {

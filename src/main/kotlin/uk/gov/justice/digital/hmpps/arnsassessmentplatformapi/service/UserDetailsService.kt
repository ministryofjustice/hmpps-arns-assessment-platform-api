...
class UserDetailsService(
  ...
  fun findOrCreate(user: UserDetails): UserDetailsEntity = userDetailsRepository.findByUserIdAndAuthSource(user.id, user.authSource)
    ?: UserDetailsEntity(
      userId = user.id,
  ...
  fun saveAll(entities: List<UserDetailsEntity>): List<UserDetailsEntity> = userDetailsRepository.saveAll(entities)
}

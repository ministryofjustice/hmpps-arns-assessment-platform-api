...
class LifestyleAssociates(ap: AnswersProvider) : SectionMapping(ap) {
  ...
  private fun q2(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_OFFENDING_ACTIVITIES).value) {
  ...
  private fun q3(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_PEER_PRESSURE).value) {
    ap.get(Value.YES) -> "0"
    ap.get(Value.SOME) -> "1"
    ap.get(Value.NO) -> "2"
    else -> null
  ...
  private fun q4(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_MANIPULATIVE_PREDATORY_BEHAVIOUR).value) {
    ap.get(Value.YES) -> "2"
    ap.get(Value.SOME) -> "1"
    ap.get(Value.NO) -> "0"
    else -> null
  ...
  private fun q5(): Any? = when (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_STABLE_BEHAVIOUR).value) {
    ap.get(Value.YES) -> "0"

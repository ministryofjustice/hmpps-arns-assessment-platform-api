...
class NewSections(ap: AnswersProvider) : SectionMapping(ap) {
  ...
  private fun q30(): Any? {
    val sexualElements = ap.answer(Field.OFFENCE_ANALYSIS_ELEMENTS).values.orEmpty().contains(
      ap.get(
        Value.SEXUAL_ELEMENT,
      ),
    )
    val sexualMotivation = ap.answer(Field.OFFENCE_ANALYSIS_MOTIVATIONS).values.orEmpty().contains(
      ap.get(
  ...
  private fun qLinkedToRosh(): Any? = PractitionerAnalysis(
    "THINKING_BEHAVIOURS_ATTITUDES",
    ap,
  ).riskOfSeriousHarm()

  private fun qLinkedToReoffending(): Any? = PractitionerAnalysis(
    "THINKING_BEHAVIOURS_ATTITUDES",
    ap,
  ).riskOfReoffending()
  ...
  private fun qAccommodationComplete(): Any = if (ap.answer(Field.ACCOMMODATION_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qEducationEmploymentComplete(): Any = if (ap.answer(Field.EMPLOYMENT_EDUCATION_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qFinanceComplete(): Any = if (ap.answer(Field.FINANCE_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qDrugsComplete(): Any = if (ap.answer(Field.DRUG_USE_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qAlcoholCompletes(): Any = if (ap.answer(Field.ALCOHOL_USE_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qHealthWellbeingComplete(): Any = if (ap.answer(Field.HEALTH_WELLBEING_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qPersonalRelationshipsComplete(): Any = if (ap.answer(Field.PERSONAL_RELATIONSHIPS_COMMUNITY_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qThinkingBehavioursAttitudesComplete(): Any = if (ap.answer(Field.THINKING_BEHAVIOURS_ATTITUDES_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"
  ...
  private fun qOffenceAnalysisComplete(): Any = if (ap.answer(Field.OFFENCE_ANALYSIS_SECTION_COMPLETE).value == ap.get(
      Value.YES,
    )
  ) {
    "YES"

...
class OffenceAnalysis : SectionMapping() {
  ...
  private fun mapVictimAge(entry: Map<String, PersistedAnswer>): String? {
    ...
    return when ((entry[Field.OFFENCE_ANALYSIS_VICTIM_AGE.lower] as? SingleValue)?.value) {
  ...
  private fun mapVictimGender(entry: Map<String, PersistedAnswer>): String? {
    ...
    return when ((entry[Field.OFFENCE_ANALYSIS_VICTIM_SEX.lower] as? SingleValue)?.value) {
      ap.get(Value.MALE) -> "1"
  ...
  private fun mapVictimRace(entry: Map<String, PersistedAnswer>): String? {
    ...
    return when ((entry[Field.OFFENCE_ANALYSIS_VICTIM_RACE.lower] as? SingleValue)?.value) {
  ...
  private fun mapVictimRelationship(entry: Map<String, PersistedAnswer>): String? {
    ...
    return when ((entry[Field.OFFENCE_ANALYSIS_VICTIM_RELATIONSHIP.lower] as? SingleValue)?.value) {
  ...
  private fun q11t(): Any? = when (ap.answer(Field.OFFENCE_ANALYSIS_ACCEPT_RESPONSIBILITY).value) {
    ap.get(Value.YES) -> ap.answer(
      Field.OFFENCE_ANALYSIS_ACCEPT_RESPONSIBILITY_YES_DETAILS,
    ).value
    ap.get(Value.NO) -> ap.answer(
      Field.OFFENCE_ANALYSIS_ACCEPT_RESPONSIBILITY_NO_DETAILS,
    ).value
    else -> null
  }
  ...
  private fun q98(): Any? = when (ap.answer(Field.OFFENCE_ANALYSIS_RISK).value) {
    ap.get(Value.YES) -> ap.answer(
      Field.OFFENCE_ANALYSIS_RISK_YES_DETAILS,
    ).value
    ap.get(Value.NO) -> ap.answer(
      Field.OFFENCE_ANALYSIS_RISK_NO_DETAILS,
    ).value
    else -> null
  }

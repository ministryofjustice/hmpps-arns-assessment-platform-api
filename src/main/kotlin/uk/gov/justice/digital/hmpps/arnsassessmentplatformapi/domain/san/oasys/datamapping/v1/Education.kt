...
class Education(ap: AnswersProvider) : SectionMapping(ap) {
  ...
  private fun q2(): Any? = when (ap.answer(Field.EMPLOYMENT_STATUS).value) {
    ap.get(Value.UNEMPLOYED_LOOKING_FOR_WORK), ap.get(
      Value.UNEMPLOYED_NOT_LOOKING_FOR_WORK,
    ),
    -> "YES"
    ap.get(Value.EMPLOYED), ap.get(
      Value.SELF_EMPLOYED,
    ),
    -> "NO"
    ap.get(Value.RETIRED), ap.get(
  ...
  private fun getEmploymentHistory(): Any? = when (ap.answer(Field.EMPLOYMENT_HISTORY).value) {
  ...
  private fun q4(): Any? = when (ap.answer(Field.EDUCATION_TRANSFERABLE_SKILLS).value) {
    ap.get(Value.NO) -> "2"
  ...
  private fun getSeverityOf(field: uk.gov.justice.digital.hmpps.arnsassessmentplatformapi.domain.san.oasys.datamapping.Field): Int? = when (ap.answer(field).value) {
  ...
  private fun q8(): Any? = when (ap.answer(Field.HEALTH_WELLBEING_LEARNING_DIFFICULTIES).value) {
  ...
  private fun q9(): Any? = when (ap.answer(Field.EDUCATION_HIGHEST_LEVEL_COMPLETED).value) {
    ...
    ap.get(Value.NOT_SURE), null -> when (
      ap.answer(
        Field.EDUCATION_PROFESSIONAL_OR_VOCATIONAL_QUALIFICATIONS,
      ).value
    ) {
  ...
  private fun q94(): Any? = PractitionerAnalysis(
    "EMPLOYMENT_EDUCATION",
    ap,
  ).notes()

  private fun q96(): Any? = PractitionerAnalysis(
    "EMPLOYMENT_EDUCATION",
    ap,
  ).riskOfSeriousHarm()
  ...
  private fun qSC2(): Any? = when (ap.answer(Field.EDUCATION_PROFESSIONAL_OR_VOCATIONAL_QUALIFICATIONS).value) {
    ap.get(Value.YES) -> "YES"
    ap.get(Value.NO) -> "NO"
    else -> null
  }

  private fun qSC2t(): Any? = ap.answer(Field.EDUCATION_PROFESSIONAL_OR_VOCATIONAL_QUALIFICATIONS_YES_DETAILS).value

  private fun qSC3(): Any? = when (ap.answer(Field.EDUCATION_HIGHEST_LEVEL_COMPLETED).value) {
  ...
  private fun qSC4(): Any? = when (ap.answer(Field.EMPLOYMENT_STATUS).value) {
    ap.get(Value.RETIRED) -> "FULLTIME"
    ap.get(Value.EMPLOYED) -> when (
      ap.answer(
  ...
  private fun qSC8(): Any? = when (ap.answer(Field.FINANCE_MONEY_MANAGEMENT).value) {

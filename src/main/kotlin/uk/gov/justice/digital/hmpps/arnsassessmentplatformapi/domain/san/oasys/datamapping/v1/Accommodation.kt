...
class Accommodation(ap: AnswersProvider) : SectionMapping(ap) {
  ...
  private fun q3(): Any? = when (ap.answer(Field.CURRENT_ACCOMMODATION).value) {
    ap.get(Value.NO_ACCOMMODATION) -> "YES"
    ap.get(Value.TEMPORARY), ap.get(
      Value.SETTLED,
    ),
    -> "NO"
    else -> null
  ...
  private fun q4(): Any? {
    val noAccommodation = ap.answer(Field.CURRENT_ACCOMMODATION).value == ap.get(
  ...
  private fun q5(): Any? = when (ap.answer(Field.CURRENT_ACCOMMODATION).value) {
    ap.get(Value.SETTLED) -> "0"
    ap.get(Value.NO_ACCOMMODATION) -> "2"
    ap.get(Value.TEMPORARY) -> when (
      ap.answer(
  ...
  private fun q6(): Any? = when (ap.answer(Field.CURRENT_ACCOMMODATION).value) {
    ap.get(Value.NO_ACCOMMODATION) -> "2"
